...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        """
          ...
          self.epsilon, take a random action and take the best policy action
          otherwise. Note that if there are no legal actions, which is the
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        if nextLegalActions:
            nextValue = max(self.getQValue(nextState, nextAction) for nextAction in nextLegalActions)
        else:
            nextValue = 0.0
        
        temp = (reward + self.discount * nextValue) - self.getQValue(state, action)
        
        for feature, value in features.items():
            self.weights[feature] += self.alpha * temp * value
